...
const collectBody = (streamBody: any = new Uint8Array(), config: PreviouslyResolved): Promise<Uint8Array> => {
  if (streamBody instanceof Uint8Array) {
    return Promise.resolve(streamBody);
  }
  return config.streamCollector(streamBody) || Promise.resolve(new Uint8Array());
};

// Encode Uint8Array data into string with utf-8.
const collectBodyString = (streamBody: any, config: PreviouslyResolved): Promise<string> =>
  collectBody(streamBody, config).then((body) => config.utf8Encoder(body));
